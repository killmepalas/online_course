...
public class CourseController {
    ...
    public String index(@PathVariable int num, Model model) {
        ...
        if (user != null) {
            if (userService.isTeacher(user)) model.addAttribute("status", "admin");
        }
    ...
    public String show(@PathVariable("id") int id, Model model) {
        ...
        model.addAttribute("grade", courseGradeService.findByUserAndCourse(user, c));
        if (overCourseService.findOneByUserAndCourse(user, c) != null)
            model.addAttribute("overCourse", overCourseService.isUserOverCourse(user, c));
        if (user != null) {
    ...
    public String unblock(@PathVariable int id) {
        ...
        if (user != null) {
            if (userService.isAdmin(user)) {
                courseService.changeStatus(id, 1);
                return "redirect: /course/index/0";
            } else return "error/not_access";
        }
        return "error/not_auth";
